...
server.tool(
  "search_humans",
  "Search for human workers by city and/or skill. Returns people available to perform real-world tasks.",
  {
...
server.tool(
  "create_task",
  "Create a new task (bounty) for humans to complete. The task will be distributed to matching workers automatically.",
  {
  ...
  async ({ title, description, city, budget, category, spots }) => {
    ...
    if (category) body.category_slug = category;
    if (spots) body.spots = spots;
...
server.tool(
  "list_tasks",
  "Browse open tasks on the marketplace. Filter by city, category, budget range.",
  {
...
server.tool(
  "get_task",
  "Get detailed information about a specific task by ID, including status and assigned worker.",
  {
...
server.tool(
  "send_message",
  "Send a message to the worker assigned to your task. Supports text communication between AI agent and human worker.",
  {
    task_id: z.number().int().describe("Task ID"),
...
server.tool(
  "get_messages",
  "Get chat messages for a task. Use to check worker's updates and responses.",
  {
    task_id: z.number().int().describe("Task ID"),
  },
  async ({ task_id }) => {
...
server.tool(
  "get_categories",
  "List all available task categories (photo, delivery, research, cleaning, etc.)",
  {},
  async () => {
...
server.tool(
  "get_stats",
  "Get platform statistics: number of workers, tasks, completed jobs.",
  {},
  async () => {
